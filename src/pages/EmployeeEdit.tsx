...
export default function EmployeeEdit() {
  ...
  const loadEmployee = async (employeeId: number) => {
    ...
    try {
      const result = await window.electronAPI.employees.getById(employeeId);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const result = await window.electronAPI.employees.update(
        Number(id),
        employeeData
      );
      ...
      if (result.success) {
        ...
        // API에서 반환한 에러 메시지 처리
        const errorMessage = result.error || "직원 수정에 실패했습니다.";
      ...
      if (err instanceof Error) {
        setError(`직원 수정 실패: ${err.message}`);
      } else {
        setError("직원 수정 중 오류가 발생했습니다.");
      }
    } finally {
  ...
  const handleReset = () => {
    if (employee) {
      ...
      setPosition(employee.position || "REG");
